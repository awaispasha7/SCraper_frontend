...
function HotpadsPageContent() {
  ...
  const filterListings = (listings: HotpadsListing[]): HotpadsListing[] => {
    if (!searchQuery.trim()) {
      return listings
    }
    ...
    return listings.filter(listing => {
      ...
      const propertyNameRaw = listing.property_name
      if (propertyNameRaw) {
        const propertyNameStr = String(propertyNameRaw).trim()
        if (propertyNameStr && propertyNameStr !== 'null' && propertyNameStr !== 'None' && propertyNameStr !== '') {
          const propertyName = normalizeForSearch(propertyNameStr)
          if (propertyName && propertyName.includes(query)) return true
        }
      }
