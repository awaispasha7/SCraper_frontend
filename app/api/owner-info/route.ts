...
export async function GET(request: NextRequest) {
  try {
    ...
    // Parse address - format is usually: "Street Address City State ZIP"
    // Examples:
    // "1229 North Austin Boulevard Chicago Il 60651"
    // "4800 S Lake Park 2107 Chicago Il 60615" (with apartment)
    // "5210 North Delphia Avenue Unit 206 Chicago Il 60656" (with unit)
    // "4118 West 63Rd Street Chicago Il 60629" (with ordinal)
    ...
    // If parsing failed, try simpler approach - look for "Chicago"
      ...
      if (chicagoMatch && chicagoMatch.index && chicagoMatch.index > 0) {
        ...
        if (match) {
          ...
          if (cityStart > 0) {
            ...
            // If city is at start, use fallback
            address1 = address
            address2 = 'Chicago, IL'
          }
        } else {
    ...
    if (dbClient) {
      try {
        ...
        if (listingLink) {
          console.log(`   [Strategy 0] Trying to match by listing_link: "${listingLink}"`)
          const { data: linkMatch, error: linkError } = await dbClient
            .from('listings')
            .select('address, owner_emails, owner_phones, listing_link')
          ...
          if (!linkError && linkMatch) {
            ...
            console.log(`   ⚠️ No match by listing_link`)
          }
        }
        
        // SECOND: If no listing_link match, query ALL listings and match by address
        if (!listingData) {
          console.log(`   [Strategy 1] Querying ALL listings with emails/phones...`)
          // Try alternative query syntax if the first one fails
          let { data: allListings, error: queryError } = await dbClient
            .from('listings')
            .select('address, owner_emails, owner_phones, listing_link')
            .or('owner_emails.not.is.null,owner_phones.not.is.null')
          
          // If that fails, try simpler query
          if (queryError) {
            console.log(`   ⚠️ First query failed, trying simpler query...`)
            ...
            // If still no results, try phones
            if (!queryError && (!allListings || allListings.length === 0)) {
              const phoneResult = await dbClient
                .from('listings')
                .select('address, owner_emails, owner_phones, listing_link')
                .not('owner_phones', 'is', null)
              
              if (phoneResult.data) {
                allListings = phoneResult.data
                queryError = phoneResult.error
              }
            }
          }
        ...
        if (!supabaseError && listingData) {
          ...
          if (listingData.owner_emails !== null && listingData.owner_emails !== undefined) {
            if (typeof listingData.owner_emails === 'string') {
              try {
                emails = JSON.parse(listingData.owner_emails)
              } catch (e) {
                console.warn('Failed to parse owner_emails from Supabase:', e)
              }
            } else if (Array.isArray(listingData.owner_emails)) {
              emails = listingData.owner_emails
            }
          }
          
          if (listingData.owner_phones !== null && listingData.owner_phones !== undefined) {
            if (typeof listingData.owner_phones === 'string') {
              try {
                phones = JSON.parse(listingData.owner_phones)
              } catch (e) {
                console.warn('Failed to parse owner_phones from Supabase:', e)
              }
            } else if (Array.isArray(listingData.owner_phones)) {
              phones = listingData.owner_phones
            }
          }
          ...
          if (emails.length > 0 || phones.length > 0) {
            ...
            console.log(`   owner_emails value: ${JSON.stringify(listingData.owner_emails)}`)
            console.log(`   owner_phones value: ${JSON.stringify(listingData.owner_phones)}`)
          }
        } else {
