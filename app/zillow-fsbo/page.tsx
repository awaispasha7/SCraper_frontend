      ...
      const checkScraperStatus = async () => {
        try {
          ...
          if (res.ok) {
            ...
            const zillowFsboStatus = statusData.zillow_fsbo
            const isRunning = zillowFsboStatus?.status === 'running'
            
            if (!isRunning && isScraperRunning) {
              // Scraper just stopped - fetch latest data from Supabase immediately
              console.log('[Zillow FSBO] Scraper stopped! Fetching latest listings from Supabase...')
              ...
              setTimeout(async () => {
                try {
                  console.log('[Zillow FSBO] Auto-refreshing listings after scraper stop...')
                  ...
                  console.error('[Zillow FSBO] Error refreshing after scraper stop:', err)
