...
function RedfinListingsPageContent() {
  ...
  const handleDownload = (listing: RedfinListing) => {
    ...
    const escapeCSV = (value: string | number | null | undefined): string => {
      ...
      if (value === null || value === undefined || value === '' || value === 'null' || value === 'No email addresses found') {
        return ''
      }
    ...
    // Prepare CSV data with all Supabase fields
    const csvData = [
      // Header row
      ['address', 'price', 'beds', 'baths', 'square_feet', 'listing_link', 'property_type', 'county', 'lot_acres', 'owner_name', 'mailing_address', 'emails', 'phones', 'scrape_date'].join(','),
      // Data row
      [
  ...
  const filterListings = (listings: RedfinListing[]): RedfinListing[] => {
    if (!searchQuery.trim()) {
      return listings
    }
        ...
        <div className="mt-8 mb-6 text-center">
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 max-w-2xl mx-auto">
            <p className="text-lg font-bold text-gray-800">
              Showing <span className="text-red-600 text-2xl">{data.total_listings}</span> listings
            </p>
            <p className="text-sm text-gray-600 mt-2">
              Data scraped on {data.scrape_date || '2025-11-20'} from Redfin
            </p>
          </div>
        </div>
      </div>
