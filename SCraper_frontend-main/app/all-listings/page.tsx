...
interface UnifiedListing {
  ...
  source: 'fsbo' | 'trulia' | 'redfin' | 'addresses'
  scrape_date?: string
...
function AllListingsPageContent() {
  ...
  const fetchAllListings = async () => {
    try {
      ...
      const [fsboRes, truliaRes, redfinRes, addressesRes] = await Promise.all([
      ...
      const [fsboData, truliaData, redfinData, addressesData] = await Promise.all([
        fsboRes.json(),
        truliaRes.json(),
        redfinRes.json(),
        addressesRes.json()
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Listings by Source */}
        {['fsbo', 'trulia', 'redfin', 'addresses'].map((source) => {
