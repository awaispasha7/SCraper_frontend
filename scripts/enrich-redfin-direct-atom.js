/**
 * Script to enrich Redfin listings CSV with owner information directly from Atom API
 * 
 * This script reads redfin_listings.csv and for each listing:
 ...
 * 
 * Usage: node scripts/enrich-redfin-direct-atom.js
 */
...
// Configuration
const INPUT_CSV = path.join(__dirname, '..', 'redfin_listings.csv')
const OUTPUT_CSV = path.join(__dirname, '..', 'redfin_listings_enriched.csv')
...
function parseAddress(address) {
  ...
  let cleanAddress = address
    .replace(/\s*\|.*$/i, '') // Remove "| Redfin" suffix
    .trim()
  ...
  // Check if address is comma-separated (common in Redfin CSV format)
  if (address.includes(',')) {
...
// Main function
async function main() {
  console.log('🚀 Starting Redfin CSV enrichment with Atom API...')
